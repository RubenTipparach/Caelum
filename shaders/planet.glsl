...
layout(binding=0) uniform planet_vs_params {
    mat4 mvp;
    vec4 camera_offset;  // xyz = camera position to subtract (0,0,0 for legacy mode)
...
void main() {
    // Camera-relative position (subtracts camera offset for precision at large distances)
    vec3 cam_rel_pos = a_position - camera_offset.xyz;
    gl_Position = mvp * vec4(cam_rel_pos, 1.0);

    // Logarithmic depth buffer (GPU Gems style)
    // Provides uniform depth precision across massive distance ranges (0.1m to 10,000km)
    // z_bias adapts to NDC z range: -1.0 for OpenGL [-1,1], 0.0 for D3D11/Metal/WebGPU [0,1]
    ...
    fs_normal = a_normal;
    fs_color = a_color;
    fs_world_pos = a_position;  // Original world pos for lighting + fog
}
@end
...
void main() {
    ...
    vec3 surface_dir = normalize(fs_world_pos);
    ...
    // LOD debug mode: color by depth level (rainbow: red → yellow → green → cyan → blue)
    vec3 base_color = fs_color;
    if (lod_debug.x > 0.0) {
        float t = clamp(lod_debug.x / max(lod_debug.y, 1.0), 0.0, 1.0);
        // 5-stop gradient: red(0) → yellow(0.25) → green(0.5) → cyan(0.75) → blue(1.0)
    ...
    // ---- Aerial perspective fog (atmospheric haze over terrain) ----
    float dist = length(fs_world_pos - camera_pos.xyz);
    float fog_density = fog_params.x;
    float fog_max_dist = fog_params.y;
    float inscatter_strength = fog_params.z;

    // Exponential-squared fog: smooth falloff, subtle nearby, strong at distance
    ...
    // Fog color: atmospheric blue on day side, dark on night side
    ...
    // Blend terrain toward fog color with distance
